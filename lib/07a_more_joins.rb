...
def best_value
  ...
  INNER JOIN
    tracks ON tracks.album = albums.asin
  GROUP BY
    albums.asin HAVING albums.price / COUNT(*) < .50
  SQL
end
...
def expensive_tastes
  ...
  execute(<<-SQL)
  SELECT
    styles.style, (SUM(track_counts.price) / SUM(track_counts.count))
  FROM
    (SELECT
      albums.*, COUNT(song)
    FROM
  ...
  INNER JOIN
    styles ON styles.album = track_counts.asin
  GROUP BY
    styles.style
  ORDER BY
